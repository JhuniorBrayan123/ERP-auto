...
import {ActivarDocAdelanto} from "@screenplay/interactions/facturacion/ActivarDocAdelanto";

type TipoComprobante = 'BOLETA' | 'FACTURA' | 'NOTA DE VENTA';
type MetodoPago = 'efectivo' | 'cheque' | 'niubiz' | 'yape' | 'plin' | 'transferencia';

export interface DatosAdelanto {
...
export const EmitirDocumentoDeAdelanto = (datos: DatosAdelanto) => {
    ...
    fn.displayName = `Emitir ${datos.tipoComprobante} como documento de adelanto — ${datos.cliente.nombre}`;
    return fn;
};
