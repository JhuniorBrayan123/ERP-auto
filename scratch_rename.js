...
fileMappings.forEach(mapping => {
    ...
    if (fs.existsSync(oldPath)) {
        ...
        // Match test.describe('Algo', ...  and avoid modifying if it already starts with PS-X | 
        content = content.replace(/test\.describe\(\s*(['"`])(.*?)\1/g, (match, quote, title) => {
            ...
            return `test.describe(${quote}${mapping.prefix} | ${cleanTitle}${quote}`;
        });

        // Match test('Algo', ...  
        content = content.replace(/test\(\s*(['"`])(.*?)\1/g, (match, quote, title) => {
            let cleanTitle = title;
            // Quitamos tags previos al final si existen
            cleanTitle = cleanTitle.replace(new RegExp(' @' + mapping.prefix + '$'), '');
            return `test(${quote}${cleanTitle.trim()} @${mapping.prefix}${quote}`;
        });

        fs.writeFileSync(newPath, content);
        fs.unlinkSync(oldPath);
        console.log(`Moved: ${mapping.new}`);
    } else {
