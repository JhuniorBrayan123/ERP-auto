...
test.describe('Carga masiva de combos desde Excel', {tag: ['@logistica', '@productos-stock']}, () => {
    ...
    test('crear combos masivamente desde excel', async ({
        ...
        await test.step('Preparar Excel con nombre único', async () => {
            const result = await buildMassiveExcel('combos', 'masivo');
            tempFilePath = result.tempFilePath;
            textoBusqueda = result.textoBusqueda;
            console.log(`  → Nombre generado: ${result.nombreGenerado}`);
        });

        await test.step('Seleccionar tipo Combos y subir archivo', async () => {
