...
test.describe('Carga masiva de insumos desde Excel', {tag: ['@logistica', '@productos-stock']}, () => {

    test('crear insumos masivamente desde excel', async ({
                                                             page,
                                                             cargaMasiva,
                                                         }) => {
        let tempFilePath = '';
        ...
        await test.step('Preparar Excel con nombre único', async () => {
            const result = await buildMassiveExcel('insumos', 'masivo');
            tempFilePath = result.tempFilePath;
            textoBusqueda = result.textoBusqueda;
            console.log(`  → Nombre generado: ${result.nombreGenerado}`);
        });

        await test.step('Seleccionar tipo Insumos y subir archivo', async () => {
