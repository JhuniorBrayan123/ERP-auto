...
test.describe('Carga masiva de servicios desde Excel', {tag: ['@logistica', '@productos-stock']}, () => {
    ...
    test('crear servicios masivamente desde excel', async ({
        ...
        await test.step('Preparar Excel con nombre único', async () => {
            const result = await buildMassiveExcel('servicios', 'masivo');
            tempFilePath = result.tempFilePath;
            textoBusqueda = result.textoBusqueda;
            console.log(`  → Nombre generado: ${result.nombreGenerado}`);
        });

        await test.step('Seleccionar tipo Servicios y subir archivo', async () => {
