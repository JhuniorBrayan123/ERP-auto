...
test.describe('Notas de Crédito — Motivos con Monto Directo', () => {

  test('Emitir NC por descuento global desde factura', async ({ facturador }) => {
  ...
  test('Emitir NC por disminución en el valor desde factura', async ({ facturador }) => {
    const origen = await facturador.realizaYObtiene(
      EmitirComprobanteOrigen({
        tipoComprobante: TIPOS_COMPROBANTE.FACTURA,
        cliente: CLIENTES.EMPRESA_RUC_AUTO,
  ...
  test('Emitir NC por otros conceptos desde boleta', async ({ facturador }) => {
    const origen = await facturador.realizaYObtiene(
      EmitirComprobanteOrigen({
