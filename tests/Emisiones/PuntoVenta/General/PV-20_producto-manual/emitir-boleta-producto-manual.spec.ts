...
test.describe('PV-20 | Producto Manual — Emisión', {tag: ['@puntoventa', '@pv-20']}, () => {
    ...
    test('SC-01: Emitir Boleta con producto manual y pago en efectivo @PV-20.1', async ({page}) => {
        const cajero = Cajero.con(page);

        const resultado = await test.step('Given: emitir boleta con producto manual', async () => {
