...
export class InventarioPage {
  ...
  readonly inputMontoFinal: Locator;
  ...
  constructor(page: Page) {
    ...
    this.inputMontoFinal = page.getByRole("textbox", { name: "Monto final" });
   ...
   */
  async esperarCarga(timeout = 120000) {
   ...
   */
  async llenarDatosBasicos(nombre: string, monto: string) {
    await this.inputNombreProducto.click();
    await this.inputNombreProducto.fill(nombre);
    await this.inputMontoFinal.click();
    await this.inputMontoFinal.fill(monto);
  }
  ...
  async configurarEquivalenciasEstandar() {
    ...
    await this.page.getByRole('textbox', { name: 'precio base' }).click();
    await this.page.getByRole('textbox', { name: 'precio base' }).fill('11');
    ...
    await this.page.getByRole('textbox', { name: 'Monto final' }).click();
    await this.page.getByRole('textbox', { name: 'Monto final' }).fill('10');
    await this.page.getByRole('button', { name: 'Crear Equivalencia' }).click();
  ...
  async verificarAccionesPostCreacion(nombreProducto: string) {
    ...
    await this.botonIrAListaItems.click({ timeout: 10000 });
    ...
    await this.tabComprasItem.click({ timeout: 10000 });
    await this.esperarCarga();
